...
void System :: step(){  // Perform a simulation step
  ...
  else for(int i=0; i<_npart; i++) this->move(int(_rnd.Rannyu()*_npart)); // Perform a MC step on a randomly choosen particle (NB:a single step does 50 M(RT)^2 moves, so at each step the magnetization is the one after 50 Metropolis moves, not only after one Metropolis move)
...
void System :: read_configuration(){
  ...
  if(cinf.is_open()){
    ...
    if(_dist_type == 1){ // Dirac velocity distribution has to start from a fcc crystal of half size
      ...
      for(int i=0; i<_npart; i++){
        cinf >> particle >> x >> y >> z; // units of coordinates in conf.xyz is _side
        _particle(i).setposition(0, this->pbc(_side(0)*x, 0));  // _side(0)*x since config.xyz is not written in reduced units
        _particle(i).setposition(1, this->pbc(_side(1)*y, 1));  // _side(1)*y since config.xyz is not written in reduced units
        _particle(i).setposition(2, this->pbc(_side(2)*z, 2));  // _side(2)*z since config.xyz is not written in reduced units
        _particle(i).acceptmove(); // _x_old = _x_new
      }
    }
  } else {

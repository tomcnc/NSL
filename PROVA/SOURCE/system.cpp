...
void System :: initialize(){ // Initialize the System object according to the content of the input files in the ../INPUT/ directory
  ...
  while ( !input.eof() ){
    ...
    if( property == "SIMULATION_TYPE" ){
      ...
      input >> _dist_type;
        if(_dist_type == "DIRAC")         coutf << "LJ MOLECULAR DYNAMICS (NVE) SIMULATION starting from DIRAC velocity distribution"  << endl;
        else if(_dist_type == "MAXBOLTZ") coutf << "LJ MOLECULAR DYNAMICS (NVE) SIMULATION starting from MAXBOLTZ velocity distribution"  << endl;
        else{
...
void System :: initialize_velocities(){
  ...
  if(_restart){
    ...
    vec sumv(_ndim);
    sumv.zeros();
    if(_dist_type == "DIRAC"){     // Dirac delta velocity distribution (low entropic initial configuration)
      ofstream coutpvs;
      coutpvs.open("../OUTPUT/initial_velocity_distribution.dat");
      coutpvs << "#      V_BIN_CENTER:             COUNTS:" << endl;
      double v_start{1.25 * sqrt(3.0*_temp)} ; // start velocity 
    ...
    } else if(_dist_type == "MAXBOLTZ"){ // typical Maxwell Boltzmann velocity distribution initial configuration (not used in ex 4.2-4.3)
      for (int i=0; i<_npart; i++){
...
void System :: initialize_properties(){ // Initialize data members used for measurement of properties
  ...
  // added by me
  _measure_pofv         = false;
  _measure_temp_step    = false;
  _measure_tenergy_step = false;
  _measure_magnet_step  = false;
  ...
  if (input.is_open()){
    while ( !input.eof() ){
      ...
      if( property == "POTENTIAL_ENERGY" ){
        ...
        input>>_n_bins_v;
        _nprop += _n_bins_v;
        _bin_size_v = 4.5*sqrt(3.0*_temp)/(double)_n_bins_v; // max sampling velocity is set to 4.5 times mean velocity value
        _measure_pofv = true;
        _index_pofv = index_property;
        index_property += _n_bins_v;
      } else if( _dist_type == "DIRAC" ){ // file to create start simulation velocity distribution histogram only for dirac
        ofstream coutpvs;
        coutpvs.open("../OUTPUT/initial_velocity_distribution.dat");
        coutpvs << "#      V_BIN_CENTER:             COUNTS:" << endl;
        coutpvs.close();
        ...
        ofstream coutprs("../OUTPUT/magnetization_step.dat");
        coutprs << "#            M_STEP:" << endl;
        coutprs.close();
        _nprop++;
...
void System :: read_configuration(){
  ...
  if(cinf.is_open()){
    ...
    cinf >> comment;
    if(_dist_type == "DIRAC"){ // dirac velocity distribution has to start from a fcc crystal of half size
      for(int i=0; i<_npart; i++){
...
void System :: measure(){ // Measure properties
  ...
  // POFV
  // MY CODE HERE (commented is another less efficient way to check which bin has to be increased)
  if (_measure_pofv) {
    vec velocity;
    ...
    for(int i{}; i < _npart; i++){
      ...
      if(int(vel/_bin_size_v) < _n_bins_v ){
        _measurement(_index_pofv + int(vel/_bin_size_v))+= 1.0/(_npart * _bin_size_v);
      }
      /*
      for(int j{}; j < _n_bins_v; j++){
        if(vel >= j*_bin_size_v and vel < (j+1)*_bin_size_v){
          _measurement(_index_pofv + j)+= 1.0 / double(_npart * _bin_size_v); // normalized maxwell-boltzmann velocity distribution
          break;
        }
      }
      */
  ...
  if (_measure_temp_step){  // added by me
  ...
  // MAGNETIZATION /////////////////////////////////////////////////////////////
  if (_measure_magnet){ // added by me
    for(int i{}; i < _npart; i++){
  ...
  if (_measure_magnetization_step){ // added by me
    ofstream coutprs("../OUTPUT/magnetization_step.dat", ios::app);
    coutprs << setw(20) << _measurement[_index_magnet] << endl;
    coutprs.close();
  }
...
void System :: averages(int blk){
  ...
  _average     = _block_av / double(_nsteps); // non molto chiaro il motivo
  _global_av  += _average;
  ...
  if (_measure_pofv){ // added by me
    ...
    for(int i{}; i < _n_bins_v; i++){
      ...
      coutf << setw(12) << blk
            << setw(20) << _bin_size_v * 0.5 + (i * _bin_size_v) 
            << setw(20) << average 
            << setw(20) << sum_average/double(blk) 
            << setw(20) << this->error(sum_average, sum_ave2, blk) << endl;
    }
  ...
  // SUSCEPTIBILITY ////////////////////////////////////////////////////////////
  if (_measure_chi){ // added by me
    coutf.open("../OUTPUT/susceptibility.dat.dat",ios::app);
